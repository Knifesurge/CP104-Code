...
def get_customer_by_id(file_handle, id_number):
    ...
    for line in file_contents:
        # Compare id (first 4 letters of line) to given id
        if line[:5] == id_number:
            # Strip new line and split by commas, putting into list
...
def get_first_customer(file_handle):
    ...
    file_contents = file_handle.readlines()
    result = []
    earliest = '1900-00-00' # Some default value chosen to fit data
    for row in file_contents:
        signup_date = row[4]
...
def append_customer(file_handle, fields):
    ...
    -------------------------------------------------------
    """
    customer_record = ",".join(fields)
    file_handle.write(str(customer_record))
    return
